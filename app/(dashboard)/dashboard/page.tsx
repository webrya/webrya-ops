...
export default async function DashboardPage() {
  ...
  // 1. Λήψη συνόλων (Stats) για τις κάρτες
  ...
  // 2. Λήψη πρόσφατων δεδομένων
  const { data: properties } = await supabase.from('properties').select('*').limit(2)
  const { data: tasks } = await supabase.from('tasks').select('*, properties(name)').limit(3)
    ...
    <div className="space-y-12 pb-24 text-white font-sans">
      ...
      <div className="px-4 md:px-0">
        <h1 className="text-4xl font-black tracking-tighter uppercase italic">
          {dict.dashboard.overview}<span className="text-amber-500">.</span>
        </h1>
      ...
      {/* SECTION: ΣΤΑΤΙΣΤΙΚΑ (STATS) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-4 md:px-0">
        <div className="bg-zinc-950 p-8 rounded-[3rem] border border-zinc-800 shadow-2xl transition-all hover:border-amber-500/30 group">
      ...
      {/* SECTION: ΤΑ ΑΚΙΝΗΤΑ ΜΟΥ */}
      <div className="space-y-6">
        <div className="flex justify-between items-center px-4 md:px-0">
          <h2 className="text-xl font-black uppercase tracking-tight italic flex items-center gap-3">
            <BarChart3 size={20} className="text-amber-500" />
            {/* Διόρθωση γραμματικής για την ελληνική γλώσσα */}
            {lang === 'GR' ? 'ΤΑ ΑΚΙΝΗΤΑ ΜΟΥ' : `${dict.common.my_prefix} ${dict.sidebar.properties} ${dict.common.my_suffix}`}
          </h2>
          <Link href="/dashboard/properties" className="text-[10px] font-black text-zinc-500 hover:text-amber-500 uppercase tracking-[0.2em] transition-colors italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 md:px-0">
          {properties?.map((prop) => (
            <Link key={prop.id} href={`/dashboard/properties/${prop.id}`} className="bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-800 hover:border-amber-500/50 transition-all group shadow-xl flex justify-between items-center">
              <div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-3 px-4 md:px-0">
          {tasks?.map((task) => (
            <div key={task.id} className="bg-zinc-950 p-6 rounded-[2.5rem] border border-zinc-800 flex items-center justify-between group hover:border-zinc-700 transition-all">
